...
const ACTIVITY_TYPES = {PLAYING: 'Playing', STREAMING: 'Streaming', LISTENING: 'Listening to', WATCHING: 'Watching'};
const PLAYING_TYPE = "PLAYING";
 ...
 */
function findGameType(gameName, presences) {
	const types = {};
	for (const [, presence] of presences) {
        for (const activity of presence.activities) {
            if (activity.type === PLAYING_TYPE && activity.name === gameName) {
	...
	return sortedTypes[0] && sortedTypes[0][0];
}
 ...
 * @returns {Collection<string, Snowflake[]>} The categorized collection
 */
function listGames(presences) {
	...
	for (const [id, presence] of presences) {
        for (const activity of presence.activities) {
            add(activity.name, id);
            add(activity.state, id);
        }
	}
